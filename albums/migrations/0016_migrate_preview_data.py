...
class Migration:
    ...
    def backwards(self, orm):
        ...
        for video in orm.Video.objects.all():
            video.albumconvertableitem_ptr.thumbFilename = move_unique(str(video.albumconvertableitem_ptr.preview),
                                                                       os.path.join('albums_videos', 'thumbs'))
            video.albumconvertableitem_ptr.save()

        for image in orm.Image.objects.all():
            image.image = move_unique(str(image.albumconvertableitem_ptr.preview),
                                      'albums_photos')
            image.save()
