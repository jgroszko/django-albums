...
class Migration:
    ...
    def forwards(self, orm):
        ...
        # Populate album slugs
        for a in orm.Album.objects.all():
            a.album_slug = a.albumitem_ptr.slug
            a.save()

        # Make sure Album slugs are unique
        for dupe in orm.Album.objects.values('album_slug').annotate(dupes=models.Count('album_slug')).filter(dupes__gt=1):
            i = 1
            for a in orm.Album.objects.filter(album_slug=dupe['album_slug'])[1:]:
                new_slug = '%s-%d' % (a.album_slug, i)
                while(Album.objects.filter(album_slug=new_slug).count() != 0):
                    new_slug = '%s-%d' % (a.album_slug, i)
                    i = i + 1

                a.album_slug = new_slug
                a.save()
                i = i + 1

        # Populate albumconvertableitem slugs
        for ai in orm.AlbumConvertableItem.objects.all():
            ai.item_slug = ai.albumitem_ptr.slug
            ai.save()

        # Make sure AlbumConvertableItem slugs are unique
        for dupe in orm.AlbumConvertableItem.objects.values('parent', 'item_slug').annotate(dupes=models.Count('item_slug')).filter(dupes__gt=1):
            i = 1
            for aci in orm.AlbumConvertableItem.objects.filter(parent=dupe['parent'], item_slug=dupe['item_slug'])[1:]:
                new_slug = '%s-%d' % (aci.item_slug, i)
                while(AlbumConvertableItem.objects.filter(parent=dupe['parent'], item_slug=new_slug).count() != 0):
                    new_slug = '%s-%d' % (aci.item_slug, i)
                    i = i + 1

                aci.item_slug = new_slug
                aci.save()
                i = i + 1    
    ...
    def backwards(self, orm):
        ...
        for a in orm.Album.objects.all():
            a.albumitem_ptr.slug = a.album_slug
            a.albumitem_ptr.save()

        for ai in orm.AlbumConvertableItem.objects.all():
            ai.albumitem_ptr.slug = ai.item_slug
            ai.albumitem_ptr.save()
